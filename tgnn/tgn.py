...
import dgl
from modules_tgnn import MemoryModule, MemoryOperation, TemporalTransformerConv, TimeEncode, EdgeFeaturePredictor
...
class TGN(nn.Module):
    def __init__(self,
        ...
        # Replace link predictor with edge feature predictor
        self.edge_feature_predictor = EdgeFeaturePredictor(embedding_dim, edge_feat_dim)
    ...
    def embed(self, graph, blocks):
        ...
        embeddings = self.embedding_attn(emb_graph, emb_memory, emb_t)
        
        # Map embeddings back to the original graph nodes
        emb2pred = dict(zip(emb_graph.ndata[dgl.NID].tolist(), emb_graph.nodes().tolist()))
        
        # Get source and destination node IDs
        src, dst = graph.edges()
        src_node_ids = graph.ndata[dgl.NID][src]
        dst_node_ids = graph.ndata[dgl.NID][dst]
        
        # Get corresponding embeddings
        src_embeddings = embeddings[[emb2pred[int(n)] for n in src_node_ids]]
        dst_embeddings = embeddings[[emb2pred[int(n)] for n in dst_node_ids]]
        
        # Predict edge features
        edge_features = self.edge_feature_predictor(src_embeddings, dst_embeddings)
