...
# Retrieve data
data = pd.read_csv('/Users/royston/Desktop/NUS Y3S2/DSE3101/data/GeopoliticalTrade2022_cleaned.csv') 
...
def update_graph(selected_countries, num_countries, order, metric):
    ...
    fig.update_layout(
        width = 1250,
        height = 700,
        xaxis=dict(titlefont=dict(size=18), range=(filtered_df["Trade Balance"].min() * 1.25 , filtered_df["Trade Balance"].max()*1.25)),
        yaxis=dict(titlefont=dict(size=18), range=[0, filtered_df["Absolute_ideal_point_distance"].max() + 0.5]),
        plot_bgcolor="white",
        showlegend=True
    )
